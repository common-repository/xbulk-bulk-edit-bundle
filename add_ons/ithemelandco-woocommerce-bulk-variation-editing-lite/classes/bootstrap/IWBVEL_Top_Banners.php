...
class IWBVEL_Top_Banners
{
    ...
    private function __construct()
    {
        if (get_option('it_halloween_banner_dismissed', 'no') == 'no' && empty(apply_filters('it_halloween_banner', []))) {
            add_filter('it_halloween_banner', function ($plugins) {
                $plugins['iwbvel'] = 'Bulk Variations';
                return $plugins;
            });
            add_action('admin_notices', [$this, 'add_halloween_banner']);
            add_action('admin_post_iwbvel_halloween_banner_dismiss', [$this, 'halloween_banner_dismiss']);
        }
    }
    ...
    public function add_halloween_banner()
    {
        ...
        .iwbvel-dismiss-banner i{
            color:#fff;
            font-size: 16px;
            vertical-align: middle;
        }

        .iwbvel-dismiss-banner:hover,
        .iwbvel-dismiss-banner:focus{
            color:#fff;
        }

        .iwbvel-middle-button{
            border: 0;
            padding: 0 15px;
            background: #FF5C00;
            float: right;
        ...
        $output .= '<img style="float: left; margin: 15px 0 0 10px;" src="' . IWBVEL_ASSETS_URL . 'images/banner-left.png" height="60px">';
        $output .= '<img style="float: right;" src="' . IWBVEL_ASSETS_URL . 'images/banner-right.png" width="auto" height="90px">';
        $output .= '<button type="button" class="iwbvel-middle-button">GRAB NOW - TILL 5 NOV</button>';
        $output .= '<form action="' . esc_url(admin_url('admin-post.php')) . '" method="post"><input type="hidden" name="action" value="iwbvel_halloween_banner_dismiss"><button class="iwbvel-dismiss-banner" type="submit"><i class="dashicons dashicons-dismiss"></i></button></form>';
        $output .= '</a>';
        $output .= '</div></div>';
