...
namespace iwbvel\classes\providers\column;
...
class VariationColumnProvider
{
        ...
        if ((!empty($this->column_data['allowed_type']) && in_array($this->product['type'], $this->column_data['allowed_type']))) {
            if ($this->column_data['editable'] === true) {
                ...
                if (isset($this->product[$this->decoded_column_key])) {
                    $output['field'] .= (is_array($this->product[$this->decoded_column_key])) ? sprintf('%s', implode(',', $this->product[$this->decoded_column_key])) : sprintf('%s', $this->product[$this->decoded_column_key]);
                } else {
                    $output['field'] .= ' ';
                }
            }
        } else {
            $value = '<i class="iwbvel-icon-slash"></i>';
            $output['field'] .= $value;
        }
    ...
    private function get_id_column()
    {
        ...
        if (Column::SHOW_ID_COLUMN === true) {
            ...
            $delete_type = 'trash';
            $delete_label = __('Delete product', 'ithemelandco-woocommerce-bulk-variation-editing-lite');
            $restore_button = '';
            $view_button = '';
            $edit_button = '';
            ...
            if ($this->product['status'] == 'trash') {
                ...
                $delete_label = __('Delete permanently', 'ithemelandco-woocommerce-bulk-variation-editing-lite');
                $restore_button = '<button type="button" style="height: 28px;" class="iwbvel-ml5 iwbvel-button-flat iwbvel-text-green iwbvel-float-right iwbvel-restore-item-btn" data-item-id="' . esc_attr($this->product['id']) . '" title="' . __('Restore', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . '"><span class="iwbvel-icon-rotate-cw"></span></button>';
            } else {
                $view_button = '<a href="' . esc_url(get_the_permalink($this->product['id'])) . '" target="_blank" title="' . __('View on site', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . '" style="height: 28px;" class="iwbvel-item-view-icon iwbvel-float-right iwbvel-ml5"><span class="iwbvel-icon-eye1" style="vertical-align: middle;"></span></a>';
                $edit_button = '<a href="' . admin_url("post.php?post=" . esc_attr($id_for_edit) . "&action=edit") . '" target="_blank" class="iwbvel-ml5 iwbvel-float-right" title="' . __('Edit product', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . '" style="height: 28px;"><span class="iwbvel-icon-pencil" style="vertical-align: middle;"></span></a>';
            }

            $sticky_class = ($this->sticky_first_columns == 'yes') ? 'iwbvel-td-sticky iwbvel-td-sticky-id iwbvel-gray-bg' : '';
            ...
            $output .= '<label class="iwbvel-td140">';
            $output .= '<input type="checkbox" class="iwbvel-check-item" data-item-type="' . esc_attr($this->product['type']) . '" value="' . esc_attr($this->product['id']) . '" title="Select Item">';
            $output .= esc_html($this->product['id']);
            $output .= $restore_button;
            $output .= $view_button;
            $output .= '<button type="button" class="iwbvel-ml5 iwbvel-button-flat iwbvel-float-right iwbvel-text-red iwbvel-delete-item-btn" data-delete-type="' . esc_attr($delete_type) . '" data-item-id="' . esc_attr($this->product['id']) . '" title="' . $delete_label . '"><span class="iwbvel-icon-trash-2"></span></button>';
            if ($this->product['type'] != 'variation') {
                $output .= '<button title="Edit Variations" style="height: 28px;" type="button" class="iwbvel-ml5 iwbvel-button-flat iwbvel-float-right iwbvel-variation-item-btn" data-toggle="float-side-modal-after-confirm" data-target="#iwbvel-float-side-modal-variations-bulk-edit"><span class="iwbvel-icon-tree"></span></button>';
            }
            $output .= $edit_button;
            $output .= "</label>";
            $output .= "</td>";
    ...
    private function get_static_columns()
    {
        ...
        if (!empty(Column::get_static_columns())) {
            foreach (Column::get_static_columns() as $static_column) {
                $sticky_class = ($this->sticky_first_columns == 'yes') ? 'iwbvel-td-sticky iwbvel-td-sticky-title iwbvel-gray-bg' : '';
                ...
                $output .= '<span data-action="inline-editable" class="iwbvel-td160">' . esc_html($this->product[$static_column['field']]) . '</span>';
                $output .= '</td>';
            }
        }
        return $output;
    ...
    private function text_field()
    {
        ...
        return "<span data-action='inline-editable' class='iwbvel-td160'>" . sprintf('%s', $value) . "</span>";
    }

    private function textarea_field()
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-text-editor' class='iwbvel-button iwbvel-button-white iwbvel-load-text-editor' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'><i class='dashicons dashicons-edit'></i></button>";
    }
    ...
    private function image_field()
    {
        ...
        $hover_box_class = ($this->enable_thumbnail_popup == 'yes') ? 'iwbvel-thumbnail' : '';
        $output = "<span data-toggle='modal' class='{$hover_box_class}' data-target='#iwbvel-modal-image' data-id='iwbvel-" . esc_attr($this->column_key) . "-" . esc_attr($this->product['id']) . "' class='iwbvel-image-inline-edit' data-full-image-src='" . esc_url($full_size) . "' data-image-id='" . esc_attr($image_id) . "'>";
        if ($this->enable_thumbnail_popup == 'yes') {
            $output .= '<div class="iwbvel-original-thumbnail">' . $this->product[$this->decoded_column_key]['medium'] . '</div>';
        }
        $output .= $image;
        $output .= "</span>";
        return $output;
    ...
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-gallery' data-item-name='" . esc_attr($this->product['title']) . "' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-field='" . esc_attr($this->column_key) . "'>Gallery</button>";
    }
    ...
    private function regular_price_field()
    {
        ...
        $output['field'] = "<span data-action='inline-editable' class='iwbvel-numeric-content iwbvel-td120'>" . esc_html($price) . "</span><button type='button' data-toggle='modal' class='iwbvel-calculator' data-target='#iwbvel-modal-" . esc_attr($this->decoded_column_key) . "-" . esc_attr($this->product['id']) . "'></button>";
        $output['includes'] = IWBVEL_VIEWS_DIR . 'bulk_edit/columns_modals/regular_price_calculator.php';
        return $output;
    }
    ...
    private function sale_price_field()
    {
        $price = ($this->product[$this->decoded_column_key] != '') ? number_format(floatval($this->product[$this->decoded_column_key]), 2) : '';
        $output['field'] = "<span data-action='inline-editable' class='iwbvel-numeric-content iwbvel-td120'>" . esc_html($price) . "</span><button type='button' data-toggle='modal' class='iwbvel-calculator' data-target='#iwbvel-modal-" . esc_attr($this->decoded_column_key) . "-" . esc_attr($this->product['id']) . "'></button>";
        $output['includes'] = IWBVEL_VIEWS_DIR . 'bulk_edit/columns_modals/sale_price_calculator.php';
        return $output;
    }
    ...
    private function numeric_field()
    {
        ...
        return "<span data-action='inline-editable' class='iwbvel-numeric-content iwbvel-td120'>" . esc_html($value) . "</span><button type='button' data-toggle='modal' class='iwbvel-calculator' data-field='" . esc_attr($this->column_key) . "' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field-type='" . esc_attr($this->field_type) . "' data-target='#iwbvel-modal-numeric-calculator'></button>";
    }

    private function numeric_without_calculator_field()
    {
        return "<span data-action='inline-editable' class='iwbvel-numeric-content iwbvel-td120'>" . esc_html($this->product[$this->decoded_column_key]) . "</span>";
    }
    ...
    private function checkbox_dual_mode_field()
    {
        ...
        return "<label><input type='checkbox' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' value='yes' class='iwbvel-dual-mode-checkbox iwbvel-inline-edit-action' " . esc_attr($checked) . "><span>" . __('Yes', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</span></label>";
    }
    ...
    private function checkbox_field()
    {
        ...
        if (!empty($this->acf_fields[$this->decoded_column_key]['choices']) && is_array($this->acf_fields[$this->decoded_column_key]['choices'])) {
            foreach ($this->acf_fields[$this->decoded_column_key]['choices'] as $choice_key => $choice_value) {
                ...
                $output = "<label><input type='checkbox' name='" . esc_attr($this->decoded_column_key . '-' . $this->product['id']) . "' value='" . esc_attr($choice_key) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' class='iwbvel-dual-mode-checkbox iwbvel-inline-edit-action' " . esc_attr($checked) . ">" . esc_html($choice_value) . "</label>";
            }
        }
        return $output;
    }
    ...
    private function file_field()
    {
        ...
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-file' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-file-id='" . $file_id . "' data-file-url='" . $file_url . "'>Select File</button>";
    }

    private function select_files_field()
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-select-files' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>Files</button>";
    }
    ...
    private function select_products_field()
    {
        ...
        return  "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-select-products' class='iwbvel-button iwbvel-button-white' data-children-ids='" . esc_attr($children_ids) . "' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>Products</button>";
    }

    private function select_field()
    {
        $output = "<select class='iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' data-field-type='" . esc_attr($this->field_type) . "'>";
        if (!empty($this->column_data['options'])) {
            ...
            if ($this->column_data['field_type'] == 'custom_field') {
                $meta_fields = get_option('iwbvel_meta_fields', []);
    ...
    {
        $output = "<select class='iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' data-field-type='" . esc_attr($this->field_type) . "'>";
        $output .= "<option value=''>" . __('Select', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</option>";
    ...
    private function date_picker_field()
    {
        ...
        $clear_button = ($this->decoded_column_key != 'date_created') ? "<button type='button' class='iwbvel-clear-date-btn iwbvel-inline-edit-clear-date' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' value=''><img src='" . esc_url(IWBVEL_IMAGES_URL . 'calendar_clear.svg') . "' alt='Clear' title='Clear Date'></button>" : '';
        return "<input type='text' class='iwbvel-datepicker iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($date) . "'>" . sprintf('%s', $clear_button);
    }
    ...
    private function date_time_picker_field()
    {
        ...
        $clear_button = "<button type='button' class='iwbvel-clear-date-btn iwbvel-inline-edit-clear-date' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' value=''><img src='" . esc_url(IWBVEL_IMAGES_URL . 'calendar_clear.svg') . "' alt='Clear' title='Clear Date'></button>";
        return "<input type='text' class='iwbvel-datetimepicker iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($date) . "'>" . sprintf('%s', $clear_button);
    }
    ...
    private function time_picker_field()
    {
        ...
        $clear_button = "<button type='button' class='iwbvel-clear-date-btn iwbvel-inline-edit-clear-date' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' value=''><img src='" . esc_url(IWBVEL_IMAGES_URL . 'calendar_clear.svg') . "' alt='Clear' title='Clear Date'></button>";
        return "<input type='text' class='iwbvel-timepicker iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($date) . "'>" . sprintf('%s', $clear_button);
    }

    private function color_picker_field()
    {
        return "<input type='text' class='iwbvel-color-picker-field iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($this->product[$this->decoded_column_key]) . "'><button type='button' class='iwbvel-inline-edit-color-action'>" . __('Apply', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
    }
    ...
    private function multi_select_field()
    {
        if (isset($this->acf_fields[$this->column_key]['taxonomy'])) {
            ...
            if (!empty($checked) && is_array($checked)) {
                ...
                foreach ($checked as $id => $name) {
                    ...
                    $values .= '<span class="iwbvel-category-item">' . esc_html($name) . $separate . ' </span>';
                    $checked_iteration++;
                }
            }
        } else {
            $values = get_the_term_list($this->product['id'], $this->column_key, '<span class="iwbvel-category-item">', ', </span><span class="iwbvel-category-item">', '</span>');
        }
        ...
        if (mb_substr($this->decoded_column_key, 0, 3) == 'pa_') {
            ...
            $output['field'] = "<span data-toggle='modal' class='iwbvel-is-attribute-modal iwbvel-product-attribute' data-is-visible='{$is_visible}' data-is-variation='{$is_variation}' data-target='#iwbvel-modal-attribute-" . esc_attr($this->column_key) . "-" . esc_attr($this->product['id']) . "' data-item-id='" . esc_attr($this->product['id']) . "'>";
            $output['field'] .= (!empty($values)) ? strip_tags(sprintf('%s', $values), '<span><ul><label><li>') : 'No items';
            $output['field'] .= "</span>";
            $output['includes'] = IWBVEL_VIEWS_DIR . 'bulk_edit/columns_modals/product_attribute.php';
        } else {
            if (isset($this->acf_fields[$this->column_key]['taxonomy'])) {
                $output['field'] = "<span data-toggle='modal' class='iwbvel-is-taxonomy-modal iwbvel-acf-taxonomy-multi-select' data-target='#iwbvel-modal-multi-select-" . esc_attr($this->column_key) . "-" . esc_attr($this->product['id']) . "' data-item-id='" . esc_attr($this->product['id']) . "'>";
                $output['field'] .= (!empty($values)) ? strip_tags(sprintf('%s', $values), '<span><ul><label><li>') : 'No items';
                $output['field'] .= "</span>";
                $output['includes'] = IWBVEL_VIEWS_DIR . 'bulk_edit/columns_modals/acf_taxonomy_multi_select.php';
            } else {
                $output['field'] = "<span data-toggle='modal' class='iwbvel-is-taxonomy-modal iwbvel-product-taxonomy' data-target='#iwbvel-modal-taxonomy-" . esc_attr($this->column_key) . "-" . esc_attr($this->product['id']) . "' data-item-id='" . esc_attr($this->product['id']) . "'>";
                $output['field'] .= (!empty($values)) ? strip_tags(sprintf('%s', $values), '<span><ul><label><li>') : 'No items';
                $output['field'] .= "</span>";
                $output['includes'] = IWBVEL_VIEWS_DIR . 'bulk_edit/columns_modals/product_taxonomy.php';
            }
        }
    ...
    private function yith_product_badge_field()
    {
        ...
        if (defined("YITH_WCBM_INIT")) {
            ...
            $output = "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-product-badges' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . __('Badges', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
        } else {
            // is free plugin - single
            $product_badges = get_posts(['post_type' => 'yith-wcbm-badge', 'posts_per_page' => -1, 'order' => 'ASC']);
            $output = "<select class='iwbvel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-item-id='" . esc_attr($this->product['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' data-field-type='" . esc_attr($this->field_type) . "'>";
            $output .= "<option value=''>" . __('No badge', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</option>";
    ...
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-ithemeland-badge' class='iwbvel-button iwbvel-button-white iwbvel-ithemeland-badge-button' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . __('iThemeland badge', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
    }

    private function yikes_custom_product_tabs_field()
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-yikes-custom-product-tabs' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . __('Custom Tabs', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
    }

    private function it_wc_dynamic_pricing_all_fields_field()
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-it-wc-dynamic-pricing-all-fields' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "'data-item-type='" . esc_attr($this->product['type']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . __('All Fields', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
    }

    private function it_pricing_rules_product_field()
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-it-wc-dynamic-pricing' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . __('Set Price', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
    }

    private function it_wc_dynamic_pricing_select_roles_field()
    {
        return "<button type='button' data-toggle='modal' data-target='#iwbvel-modal-it-wc-dynamic-pricing-select-roles' class='iwbvel-button iwbvel-button-white' data-item-id='" . esc_attr($this->product['id']) . "' data-item-name='" . esc_attr($this->product['title']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . __('Select Roles', 'ithemelandco-woocommerce-bulk-variation-editing-lite') . "</button>";
    }
