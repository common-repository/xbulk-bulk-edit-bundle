...
class Product
{
    ...
    public function get_product_statuses()
    {
        ...
        $statuses['trash'] = esc_html__('Trash', 'ithemelandco-woocommerce-bulk-variation-editing-lite');
        return $statuses;
    }
    ...
    public function get_products_list($data, $active_page)
    {
        ...
        $args = \iwbvel\classes\helpers\Setting::get_arg_order_by(sanitize_text_field($column_name), [
        ...
        $products_list = Render::html(IWBVEL_VIEWS_DIR . 'data_table/items.php', compact('parent_only', 'item_provider', 'display_full_columns_title', 'items', 'columns', 'sort_type', 'sort_by', 'show_id_column', 'next_static_columns', 'columns_title', 'sticky_first_columns'));
    ...
    public function get_products_rows($product_ids)
    {
        ...
        $settings = $settings_repository->get_settings();
        $sticky_first_columns = $settings['sticky_first_columns'];
        $column_provider = VariationColumnProvider::get_instance();
        $show_id_column = $this->column_repository::SHOW_ID_COLUMN;
        $next_static_columns = $this->column_repository::get_static_columns();
        $columns = $this->get_active_columns();
    ...
    private function get_status_filters()
    {
        ...
        return Render::html(IWBVEL_VIEWS_DIR . "bulk_edit/status_filters.php", compact('product_counts_by_status', 'product_statuses'));
    }
    ...
    public function get_grouped_taxonomies()
    {
        ...
        foreach ($taxonomies as $taxonomy) {
            if (taxonomy_exists($taxonomy->name) && !in_array($taxonomy->name, $default_taxonomies)) {
                $tax_type = \iwbvel\classes\helpers\Meta_Fields::get_taxonomy_type($taxonomy->name);
    ...
    public function get_product_counts_group_by_status()
    {
        $output = [];
        $all = 0;
        $result = $this->wpdb->get_results("SELECT post_status AS `status`,COUNT(*) AS `count` FROM {$this->wpdb->posts} WHERE post_type = 'product' AND post_status NOT IN ('auto-draft') GROUP BY post_status", ARRAY_A);
    ...
    {
        $tax_classes[''] = esc_html__('Standard', 'ithemelandco-woocommerce-bulk-variation-editing-lite');
    ...
    public function get_product_ids_with_like_names($product_ids = [])
    {
        ...
        return $this->wpdb->get_results("SELECT GROUP_CONCAT(ID) as product_ids, count(*) as product_count FROM {$this->wpdb->posts} WHERE post_type = 'product' AND post_status != 'trash' {$product_id_query} GROUP BY post_title HAVING product_count > 1 ORDER BY product_count", ARRAY_A);
    }
