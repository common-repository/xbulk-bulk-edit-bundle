        ...
        if (is_array($array)) {
            if (count($array) > 0) {
                foreach ($array as $key => $value) {
                    $sanitized[sanitize_text_field($key)] = (is_array($value)) ? self::array($value) : wp_kses(stripslashes($value), self::allowed_html());
                }
            }
        } else {
            $sanitized = wp_kses(stripslashes($array), self::allowed_html());
        }
        return $sanitized;
    }
