...
class Generator
{
    public static function license_hash($license_data, $product_id)
    {
        return (empty($license_data) || !isset($license_data['license_key']) || !isset($license_data['email']))
            ...
            : md5($license_data['license_key'] . sanitize_text_field($product_id) . $license_data['email'] . sanitize_text_field($_SERVER['SERVER_NAME']));
    }
    ...
    public static function help_icon($text)
    {
        $output = "";
        if (!empty($text)) {
            $output = "<span class='iwbvel-field-help dashicons dashicons-info' title='{$text}'></span>";
        }
        return sprintf('%s', $output);
    }
