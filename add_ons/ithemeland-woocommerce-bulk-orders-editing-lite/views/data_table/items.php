...
<table id="wobel-items-list" class="widefat">
        ...
        <tr>
            <?php if (isset($show_id_column) && $show_id_column === true) : ?>
                ...
                if ('id' == $sort_by) {
                    ...
                    $sortable_icon = "<img src='" . esc_url(WOBEL_IMAGES_URL . "/sortable.png") . "' alt=''>";
                }
                ?>
                <th class="wobel-td70 <?php echo ($sticky_first_columns == 'yes') ? 'wobel-td-sticky wobel-td-sticky-id' : ''; ?>">
                    <input type="checkbox" class="wobel-check-item-main" title="<?php esc_attr_e('Select All', 'ithemeland-woocommerce-bulk-orders-editing-lite'); ?>">
                    <label data-column-name="id" class="wobel-sortable-column"><?php esc_html_e('ID', 'ithemeland-woocommerce-bulk-orders-editing-lite'); ?><span class="wobel-sortable-column-icon"><?php echo wp_kses($sortable_icon, Sanitizer::allowed_html_tags()); ?></span></label>
                </th>
            <?php endif; ?>
            <?php if (!empty($next_static_columns)) : ?>
                <?php foreach ($next_static_columns as $static_column) : ?>
                    ...
                    ?>
                    <th data-column-name="<?php echo esc_attr($static_column['field']) ?>" class="wobel-sortable-column wobel-td120 <?php echo ($sticky_first_columns == 'yes') ? 'wobel-td-sticky wobel-td-sticky-title' : ''; ?>"><?php echo esc_html($static_column['title']); ?><span class="wobel-sortable-column-icon"><?php echo wp_kses($sortable_icon, Sanitizer::allowed_html_tags()); ?></span></th>
                <?php endforeach; ?>
            <?php endif; ?>
            ...
            ?>
                    <th data-column-name="<?php echo esc_attr($column_name); ?>" <?php echo (!empty($column['sortable'])) ? 'class="wobel-sortable-column"' : ''; ?>><?php echo (!empty($title)) ? "<span class='wobel-column-title dashicons dashicons-info' title='" . esc_attr($title) . "'></span>" : "" ?> <?php echo esc_html($column_title); ?> <span class="wobel-sortable-column-icon"><?php echo wp_kses($sortable_icon, Sanitizer::allowed_html_tags()); ?></span></th>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <tbody>
        <?php if (!empty($items_loading)) : ?>
            <tr>
                <td colspan="100%" class="wobel-text-alert"><?php esc_html_e('Loading ...', 'ithemeland-woocommerce-bulk-orders-editing-lite'); ?></td>
            </tr>
        <?php
        ...
        ?>
            <tr>
                <td colspan="100%" class="wobel-text-alert"><?php esc_html_e('No Data Available!', 'ithemeland-woocommerce-bulk-orders-editing-lite'); ?></td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
