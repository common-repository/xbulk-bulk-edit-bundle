...
use wobel\classes\repositories\order\Order_Table_Based;
use wobel\classes\repositories\order\Post_Table_Based;
...
class Order
{
    ...
    public function get_ids_by_custom_query($join, $where, $type = 'shop_order')
    {
        ...
        $orders = $wpdb->get_results($query, ARRAY_N); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    {
        if ($this->hpos_sync_enabled()) {
            $object = new Order_Table_Based();
        } else {
            if ($this->hpos_enabled()) {
                $object = new Order_Table_Based();
            } else {
                $object = new Post_Table_Based();
            }
        }

        return $object->get_orders($args);
    }
    ...
    public function hpos_sync_enabled()
    {
    ...
    public function get_orders_list($data, $active_page)
    {
        ...
        $orders_args = Order_Helper::set_filter_data_items($data, [
            'orderby' => sanitize_text_field($sort_by),
            'order' => sanitize_text_field($sort_type),
            'limit' => $current_settings['count_per_page'],
            'paged' => $active_page,
            'paginate' => true,
            'return' => 'ids',
        ]);
    ...
    public function order_to_array($order_object)
    {
        ...
        $post_meta = $order_object->get_meta_data();
        ...
        return [
            ...
            '_order_stock_reduced' => ($order_object->get_order_stock_reduced()) ? 'yes' : 'no',
            '_recorded_sales' => ($order_object->get_recorded_sales()) ? 'yes' : 'no',
            ...
            '_order_tax' => $order_object->get_total_tax(),
    ...
    public function order_to_array_for_export($order_object)
    {
        ...
        $order_notes = !empty($notes) && is_array($notes) ? serialize($notes) : ' ';
        ...
        $order_status = $order_object->get_status();
        $order_status = ($order_status != 'trash') ? 'wc-' . $order_status : $order_status;
        $user_object = $order_object->get_user();
        $customer_username = (!empty($user_object) && $user_object instanceof \WP_User) ? $user_object->user_login : 'guest';
        ...
        return [
            ...
            '_recorded_sales' => ($order_object->get_recorded_sales()) ? 'yes' : 'no',
            'customer_ip_address' => $order_object->get_customer_ip_address(),
            'customer_user' => $customer_username,
            'customer_user_agent' => ($customer_agent_name instanceof \WP_User) ? $customer_agent_name->user_nicename : '',
            'date_paid' => $order_object->get_date_paid(),
            'date_completed' => $order_object->get_date_completed(),
            'order_total' => $order_object->get_total(),
    ...
    public function get_order_counts_group_by_status()
    {
        ...
        if ($this->hpos_sync_enabled()) {
            $result = $wpdb->get_results("SELECT `status` ,COUNT(*) AS 'count' FROM {$wpdb->prefix}wc_orders WHERE `status` NOT IN ('auto-draft') GROUP BY `status`", ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        } else {
            if ($this->hpos_enabled()) {
                $result = $wpdb->get_results("SELECT `status` ,COUNT(*) AS 'count' FROM {$wpdb->prefix}wc_orders WHERE `status` NOT IN ('auto-draft') GROUP BY `status`", ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            } else {
                $result = $wpdb->get_results("SELECT post_status AS 'status',COUNT(*) AS 'count' FROM {$wpdb->posts} WHERE post_type = 'shop_order' AND post_status NOT IN ('auto-draft') GROUP BY post_status", ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            }
        }
    ...
    public function get_status_color($status)
    {
        ...
        $result = $wpdb->get_row($wpdb->prepare($query, str_replace('wc-', '', sanitize_text_field($status)))); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
