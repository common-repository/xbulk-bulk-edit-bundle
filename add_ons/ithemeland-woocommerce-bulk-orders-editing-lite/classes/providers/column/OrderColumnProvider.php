...
class OrderColumnProvider
{
    ...
    private function get_id_column()
    {
        ...
        if (Column::SHOW_ID_COLUMN === true) {
            ...
            if ($this->order['order_status'] == 'trash') {
                ...
                $restore_button = '<button type="button" style="height: 28px;" class="wobel-ml5 wobel-button-flat wobel-float-right wobel-text-green wobel-restore-item-btn" data-item-id="' . esc_attr($this->order['id']) . '" title="' . esc_html__('Restore', 'ithemeland-woocommerce-bulk-orders-editing-lite') . '"><span class="wobel-icon-rotate-cw"></span></button>';
            } else {
                $edit_button = '<a href="' . admin_url("post.php?post=" . esc_attr($this->order['id']) . "&action=edit") . '" target="_blank" style="height: 28px;" class="wobel-ml5 wobel-float-right" title="Edit Order"><span class="wobel-icon-pencil" style="vertical-align: middle;"></span></a>';
            }
            ...
            $output .= '<input type="checkbox" class="wobel-check-item" value="' . esc_attr($this->order['id']) . '" title="Select Order">';
            $output .= esc_html($this->order['id']);
            $output .= $restore_button;
            $output .= '<button type="button" class="wobel-ml5 wobel-button-flat wobel-text-red wobel-float-right wobel-delete-item-btn" data-delete-type="' . esc_attr($delete_type) . '" data-item-id="' . esc_attr($this->order['id']) . '" title="' . $delete_label . '"><span class="wobel-icon-trash-2"></span></button>';
            $output .= $edit_button;
            $output .= "</label>";
            $output .= "</td>";
        }
    ...
    {
        return "<button type='button' data-toggle='modal' data-target='#wobel-modal-text-editor' class='wobel-button wobel-button-white wobel-load-text-editor wobel-td160' data-item-id='" . esc_attr($this->order['id']) . "' data-item-name='#" . esc_attr($this->order['id']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>Content</button>";
    }
    ...
    private function image_field()
    {
        ...
        return "<span data-toggle='modal' data-target='#wobel-modal-image' data-id='wobel-" . esc_attr($this->column_key) . "-" . esc_attr($this->order['id']) . "' class='wobel-image-inline-edit' data-full-image-src='" . esc_url($full_size) . "' data-image-id='" . esc_attr($image_id) . "'>" . $image . "</span>";
    }

    private function numeric_with_calculator_field()
    {
        return "<span data-action='inline-editable' class='wobel-numeric-content wobel-td120'>" . esc_html($this->order[$this->decoded_column_key]) . "</span><button type='button' data-toggle='modal' class='wobel-calculator' data-field='" . esc_attr($this->column_key) . "' data-item-id='" . esc_attr($this->order['id']) . "' data-item-name='#" . esc_attr($this->order['id']) . "' data-field-type='" . esc_attr($this->field_type) . "' data-target='#wobel-modal-numeric-calculator'></button>";
    }

    private function numeric_field()
    {
        return "<span data-action='inline-editable' class='wobel-numeric-content wobel-td120'>" . esc_attr($this->order[$this->decoded_column_key]) . "</span>";
    }
    ...
    private function checkbox_dual_model_field()
    {
        ...
        return "<label><input type='checkbox' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' value='yes' class='wobel-dual-mode-checkbox wobel-inline-edit-action' " . wp_kses($checked, Sanitizer::allowed_html_tags()) . "><span>" . esc_html__('Yes', 'ithemeland-woocommerce-bulk-orders-editing-lite') . "</span></label>";
    }

    private function file_field()
    {
        $file_id = (isset($this->order[$this->decoded_column_key])) ? intval(esc_attr($this->order[$this->decoded_column_key])) : null;
        $file_url = wp_get_attachment_url($file_id);
        $file_url = !empty($file_url) ? esc_url($file_url) : '';
        return "<button type='button' data-toggle='modal' data-target='#wobel-modal-file' class='wobel-button wobel-button-white' data-item-id='" . esc_attr($this->order['id']) . "' data-item-name='#" . esc_attr($this->order['id']) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-file-id='" . $file_id . "' data-file-url='" . $file_url . "'>Select File</button>";
    }

    private function order_status_field()
    {
        $statuses = $this->order_repository->get_order_statuses();
        $output = "<select class='wobel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-item-id='" . esc_attr($this->order['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' data-field-type='" . esc_attr($this->field_type) . "'>";
    ...
    {
        $date = (!empty($this->order[$this->decoded_column_key])) ? gmdate('Y/m/d', strtotime($this->order[$this->decoded_column_key])) : '';
        $clear_button = ($this->decoded_column_key != 'post_date') ? "<button type='button' class='wobel-clear-date-btn wobel-inline-edit-clear-date' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' value=''><img src='" . esc_url(WOBEL_IMAGES_URL . 'calendar_clear.svg') . "' alt='Clear' title='Clear Date'></button>" : '';
        return "<input type='text' class='wobel-datepicker wobel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($date) . "'>" . wp_kses($clear_button, Sanitizer::allowed_html_tags());
    }

    private function datetime_field()
    {
        $date = (!empty($this->order[$this->decoded_column_key])) ? gmdate('Y/m/d H:i', strtotime($this->order[$this->decoded_column_key])) : '';
        $clear_button = "<button type='button' class='wobel-clear-date-btn wobel-inline-edit-clear-date' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' value=''><img src='" . esc_url(WOBEL_IMAGES_URL . 'calendar_clear.svg') . "' alt='Clear' title='Clear Date'></button>";
        return "<input type='text' class='wobel-datetimepicker wobel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($date) . "'>" . wp_kses($clear_button, Sanitizer::allowed_html_tags());
    }

    private function time_field()
    {
        $date = (!empty($this->order[$this->decoded_column_key])) ? gmdate('H:i', strtotime($this->order[$this->decoded_column_key])) : '';
        $clear_button = "<button type='button' class='wobel-clear-date-btn wobel-inline-edit-clear-date' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' value=''><img src='" . esc_url(WOBEL_IMAGES_URL . 'calendar_clear.svg') . "' alt='Clear' title='Clear Date'></button>";
        return "<input type='text' class='wobel-timepicker wobel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($date) . "'>" . wp_kses($clear_button, Sanitizer::allowed_html_tags());
    }

    private function color_field()
    {
        return "<input type='text' class='wobel-color-picker-field wobel-inline-edit-action' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' title='Select " . esc_attr($this->column_data['label']) . "' value='" . esc_attr($this->order[$this->decoded_column_key]) . "'><button type='button' class='wobel-inline-edit-color-action'>" . esc_html__('Apply', 'ithemeland-woocommerce-bulk-orders-editing-lite') . "</button>";
    }
    ...
    private function customer_field()
    {
        ...
        return "<a href='javascript:;'  class='wobel-td160 wobel-customer-details' {$modal} data-item-id='" . esc_attr($this->order['id']) . "' data-item-name='" . esc_Attr($this->order[$this->column_key]) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . esc_html($this->order[$this->column_key]) . "</a>";
    }
}
