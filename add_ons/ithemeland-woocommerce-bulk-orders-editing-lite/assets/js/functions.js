...
function wobelOpenTab(item) {
    ...
    wobelParentContent.find("li button.selected").removeClass("selected");
    if (wobelTabItem.closest('.wobel-sub-tab').length > 0) {
        wobelTabItem.closest('li.wobel-has-sub-tab').find('button').first().addClass("selected");
    } else {
    ...
    if (item.closest('.wobel-tabs-list').attr('data-content-id') && item.closest('.wobel-tabs-list').attr('data-content-id') == 'wobel-main-tabs-contents') {
        jQuery('.wobel-tabs-list[data-content-id="wobel-main-tabs-contents"] li[data-depend] button').not('.wobel-tab-item').addClass('disabled');
        jQuery('.wobel-tabs-list[data-content-id="wobel-main-tabs-contents"] li[data-depend="' + wobelDataBox + '"] button').removeClass('disabled');
    }
...
function wobelCheckFilterFormChanges() {
    ...
    if (isChanged === true) {
        ...
        jQuery('.wobel-top-nav-status-filter button[data-status="all"]').addClass('active');
    }
}
...
function wobelGetCurrentPage() {
    return jQuery('.wobel-top-nav-filters .wobel-top-nav-filters-paginate button.current').attr('data-index');
}
...
function setFilterValues(response) {
    ...
    if (filterData) {
        jQuery('.wobel-top-nav-status-filter button').removeClass('active');
        jQuery.each(filterData, function (key, values) {
            switch (key) {
                case 'order_status':
                    if (values.value[0]) {
                        jQuery('.wobel-top-nav-status-filter button[data-status="' + values.value[0] + '"]').addClass('active');
...
function wobelGetProSearchData() {
    ...
    data = {
        ...
        created_via: {
            operator: jQuery('#wobel-filter-form-order-created-via-operator').val(),
            value: jQuery('#wobel-filter-form-order-created-via').val(),
        },
        payment_method: {
