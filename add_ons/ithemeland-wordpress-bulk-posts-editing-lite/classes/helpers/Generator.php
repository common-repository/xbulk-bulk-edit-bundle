...
class Generator
{
    ...
    public static function help_icon($text)
    {
        $output = "";
        if (!empty($text)) {
            $output = "<span class='wpbel-field-help dashicons dashicons-info' title='{$text}'></span>";
        }
        return sprintf('%s', $output);
    }
    ...
    public static function select_field($attributes, $options, $first_select_option = false)
    {
        ...
        if ($first_select_option) {
            $output .= "<option value=''>" . __('Select', 'ithemeland-wordpress-bulk-posts-editing-lite') . "</option>";
        }
