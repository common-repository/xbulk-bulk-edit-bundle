...
class WPBEL_Top_Banners
{
    ...
    private function __construct()
    {
        if (get_option('it_halloween_banner_dismissed', 'no') == 'no' && empty(apply_filters('it_halloween_banner', []))) {
            add_filter('it_halloween_banner', function ($plugins) {
                $plugins['wpbel'] = 'Bulk Posts';
                return $plugins;
            });
            add_action('admin_notices', [$this, 'add_halloween_banner']);
            add_action('admin_post_wpbel_halloween_banner_dismiss', [$this, 'halloween_banner_dismiss']);
        }
    }
    ...
    public function add_halloween_banner()
    {
        ...
        .wpbel-dismiss-banner i{
            color:#fff;
            font-size: 16px;
            vertical-align: middle;
        }

        .wpbel-dismiss-banner:hover,
        .wpbel-dismiss-banner:focus{
            color:#fff;
        }
        ...
        $output .= '<img style="float: left; margin: 15px 0 0 10px;" src="' . WPBEL_ASSETS_URL . 'images/banner-left.png" height="60px">';
        $output .= '<img style="float: right;" src="' . WPBEL_ASSETS_URL . 'images/banner-right.png" width="auto" height="90px">';
        $output .= '<button type="button" class="wpbel-middle-button">GRAB NOW - TILL 5 NOV</button>';
        $output .= '<form action="' . esc_url(admin_url('admin-post.php')) . '" method="post"><input type="hidden" name="action" value="wpbel_halloween_banner_dismiss"><button class="wpbel-dismiss-banner" type="submit"><i class="dashicons dashicons-dismiss"></i></button></form>';
        $output .= '</a>';
        $output .= '</div></div>';
