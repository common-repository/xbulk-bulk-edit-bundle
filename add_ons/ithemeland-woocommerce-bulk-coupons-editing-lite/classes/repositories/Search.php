...
class Search
{
    private $filter_profile_option_name = "wccbel_filter_profile";
    private $use_always_table = "wccbel_filter_profile_use_always";
    private $current_data_option_name = "wccbel_filter_profile_current_data";
    ...
    public function update_current_data($current_data)
    {
        ...
        foreach ($current_data as $data_key => $data_value) {
            $old_current_data[sanitize_text_field($data_key)] = esc_sql($data_value);
        }
    ...
    public function set_default_item()
    {
        $default_item['default'] = [
            'name' => __('All Coupons', 'ithemeland-woocommerce-bulk-coupons-editing-lite'),
