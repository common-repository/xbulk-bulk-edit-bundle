...
class History
{
    ...
    public function __construct()
    {
        ...
        $this->sub_system = 'woocommerce_coupons';
    }

    public static function get_operation_types()
    {
        return [
            self::BULK_OPERATION => __('Bulk Operation', 'ithemeland-woocommerce-bulk-coupons-editing-lite'),
            self::INLINE_OPERATION => __('Inline Operation', 'ithemeland-woocommerce-bulk-coupons-editing-lite'),
        ];
    }
    ...
    public function get_histories($where = [], $limit = 10, $offset = 0)
    {
        ...
        return $this->wpdb->get_results("SELECT * FROM {$this->history_table} history WHERE {$where_items} ORDER BY history.id DESC {$limit_offset}"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    }
    ...
    public function get_history_count($where = [])
    {
        ...
        $result = $this->wpdb->get_results("SELECT COUNT(id) as 'count' FROM {$this->history_table} history WHERE {$where_items} ORDER BY history.id DESC"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    {
        return $this->wpdb->get_results("SELECT * FROM {$this->history_table} WHERE reverted = 0 AND sub_system = '{$this->sub_system}' ORDER BY id DESC LIMIT 1"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    }

    public function get_latest_reverted()
    {
        return $this->wpdb->get_results("SELECT * FROM {$this->history_table} WHERE reverted = 1 AND sub_system = '{$this->sub_system}' ORDER BY id DESC LIMIT 1"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    }
    ...
    {
        $this->wpdb->query("DELETE FROM {$this->history_table} WHERE sub_system = '{$this->sub_system}'"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    }
    ...
    public function revert($history_id)
    {
        ...
        foreach ($history_items as $item) {
            $coupon_ids = [intval($item->historiable_id)];
