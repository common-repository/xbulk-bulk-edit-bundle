...
class Generator
{
    ...
    public static function help_icon($text)
    {
        $output = "";
        if (!empty($text)) {
            $output = '<span class="wccbel-field-help dashicons dashicons-info" title="' . esc_attr($text) . '"></span>';
        }
        return sprintf('%s', $output);
    }
    ...
    public static function select_field($attributes, $options, $first_select_option = false)
    {
        ...
        if ($first_select_option) {
            $output .= "<option value=''>" . __('Select', 'ithemeland-woocommerce-bulk-coupons-editing-lite') . "</option>";
        }
